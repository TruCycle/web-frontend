...
export function Button({
  ...
  const variantClasses =
    variant === 'primary'
      ...
      : variant === 'danger'
      ? 'bg-[#F43F5E] text-tc-action-secondaryText ring-1 ring-tc-action-secondaryRing hover:bg-[#E11D48] focus-visible:ring-tc-action-secondaryRing'
      : 'bg-[#F8F8F9] text-[#121212] hover:bg-slate-90'

  return (
    <button
      className={classNames(
        'inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-md font-medium transition focus-visible:outline-none focus-visible:ring-2 disabled:cursor-not-allowed disabled:opacity-60',
        variantClasses,
        className,
      )}
      type={type}
